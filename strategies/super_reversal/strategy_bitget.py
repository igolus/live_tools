...
all_balance = bitget.get_all_balance()
symbol_balance = {}
usdt_balance = all_balance["USDT"]
usdt_all_balance = usdt_balance
for k in all_balance:
    if k in symbol_list:
        if all_balance[k] > bitget.get_min_order_amount(k+"USDT"):
            symbol_balance[k] = bitget.convert_amount_to_precision(k+"USDT", all_balance[k])
...
for symbol in symbol_balance:
    ...
    if symbol_balance[symbol] == 0:
        if row["super_trend_direction"] == True and row["ema_short"] > row["ema_long"]:
            buy_limit_price = bitget.convert_price_to_precision(pair, row["ema_short"])
            buy_quantity_in_usd = usdt_all_balance * params_coin[pair]["wallet_exposure"]
            buy_quantity = bitget.convert_amount_to_precision(pair, (buy_quantity_in_usd / buy_limit_price))
            print(f"Buy limit on {pair} of {buy_quantity} at the price of {buy_limit_price}$")
            try:
                bitget.place_limit_order(pair, "Buy", buy_quantity, buy_limit_price)
            except Exception as e:
                print(f"    Error: {e}")
    elif symbol_balance[symbol] > 0:
        if row["super_trend_direction"] == False or row["ema_short"] < row["ema_long"]:
            sell_limit_price = bitget.convert_price_to_precision(pair, row["ema_short"])
            sell_quantity = bitget.convert_amount_to_precision(pair, symbol_balance[symbol])
            print(f"Sell limit on {pair} of {sell_quantity} at the price of {sell_limit_price}$")
            try:
                bitget.place_limit_order(pair, "Sell", sell_quantity, sell_limit_price)
            except Exception as e:
                print(f"    Error: {e}")
